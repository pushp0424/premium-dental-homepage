-- One-time setup script for a fresh Turso database.
-- Run this once in the Turso dashboard's SQL Shell (or `turso db shell <name>`).
-- Matches prisma/schema.prisma as of the document-content-blob migration —
-- Prisma's own migrate engine can't reach libsql:// URLs directly, so this
-- combines both migrations into the final table shape in one pass.
